...
class WxPayApi
{
	...
	public static function notify($callback, &$msg)
	{
		...
		//file_put_contents (dirname ( __FILE__ ).DIRECTORY_SEPARATOR."log/log.txt", date ( "Y-m-d H:i:s" ) . " 回调返回" . $xml. "\r\n", FILE_APPEND );
		//如果返回成功则验证签名
		try {
...
//		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		...
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, '6672');
		curl_setopt($ch, CURLOPT_TIMEOUT, '6672');
		...
		$data = curl_exec($ch);
		if (curl_errno($ch)) {
		  file_put_contents (dirname ( __FILE__ ).DIRECTORY_SEPARATOR."log/log.txt", date ( "Y-m-d H:i:s" ) . " 支付异常捕获" . curl_error($ch). "\r\n", FILE_APPEND );
