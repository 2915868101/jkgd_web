...
$URL="http://113.246.243.65:9998/EasyOnlineService/services/hsWebService?wsdl"; 
$trade='ydd-gh'.date('YmdHis').mt_rand(100,10000).mt_rand(100,10000);
$arr=$_GET;
Log::DEBUG("                                            ");
Log::DEBUG($trade."  挂号入参   ".json_encode($arr));
...
if($mobilesave=="success"){
	...
	$input->SetNotify_url('http://www.hnwisdom.com/BMFW/dist/public/weixin/wxpay/example/notify_bmfw_chizhou.php');
...
<head>
    ...
    var url="http://113.246.243.65:9998/EasyOnlineService/services/hsWebService?wsdl";
	        ...
	        timeout: 60000, 
	     	success:function(data){
				...
				if(info.FLAG!=null&&info.FLAG!=undefined){
				    window.location.href="http://www.gdxyy.org.cn/_weixin/dist/#/yygh_yycg?trade="+trade; 
				}else{
					window.history.back(-1); 
				}					
			},
		...
		WeixinJSBridge.invoke(
			...
			function(res){	
				...
				if(res.err_msg == "get_brand_wcpay_request:cancel"){//用户取消支付或者余额不足支付失败   微信返回不一定准确
					...
					window.location.href="http://www.hnwisdom.com/BMFW/dist/#/yygh_yycg?trade="+trade; 
				}			
			}
		);
	}
