...
if(empty($_REQUEST["product"])){
  $mobilesave='对不起，没有获取到您的支付信息';
  Log::DEBUG('-------支付信息：'.$_REQUEST["product"].'---------金额、订单号异常'); 
}
else{
	   //解析product中的json字符串 
	   //格式: product='{"brxm": "李丽","money": "3.00","orgId": "gd001","phone": "18974838847","zfxx": "挂号","trade":"458023534345580005423"';
	    Log::DEBUG('-------支付信息：入参'.json_encode($_REQUEST["product"],true)); 
		$product=json_decode($_REQUEST["product"],true);
		$trade= $product['trade'];
		$money= $product['money'];
		$orgId= $product['orgId'];
		$zfxx= $product['zfxx'];	
		Log::DEBUG($trade." *********支付信息：".$zfxx."***机构编码".$orgId."***金额：".$money);
