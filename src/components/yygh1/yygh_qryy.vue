<template>
  <div class="yygh_qryy g-wrap">
    <van-cell-group>
      ...
      <van-cell title="预约时间">
        <div class="s-c303133">{{reserveForm.YYRQ}} {{reserveForm.STARTTIME}}-{{reserveForm.ENDTIME}}</div>
      </van-cell>
      <van-cell title="就诊人" is-link @click="showPopup" v-if="!hz_userinfo">
        <span class="s-c303133" v-if="reserveForm.HZXM">{{reserveForm.HZXM}}</span>
        <span v-else class="s-c999">请选择就诊人</span>
      </van-cell>
      <van-cell title="就诊人" v-else>
        <span class="s-c303133" v-if="reserveForm.HZXM">{{reserveForm.HZXM}}</span> 
      </van-cell>
    </van-cell-group>
...
export default {
  ...
  created() {
    ...
    if (!sessionStorage.getItem("hz_userinfo")) {
      t.getPatientList()
    } else { //惠州粤事通小程序跳转的不调用 患者选择
      t.hz_userinfo = JSON.parse(sessionStorage.getItem("hz_userinfo")) 
      t.reserveForm.HOSPITALCODE = sessionStorage.getItem("hz_hospitalcode")
      t.reserveForm.HZXM = t.hz_userinfo.name
      t.reserveForm.CARDNUM = t.hz_userinfo.card_number
      t.reserveForm.SJHM = t.hz_userinfo.sjhm
      t.reserveForm.DZJKM = t.hz_userinfo.DZJKM
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    doSubmit() {
      var t = this
      console.log(t.IdCard(t.reserveForm.CARDNUM,3))
      var params = {  
        MONEY: t.reserveForm.GHF,
        ORG_ID: t.reserveForm.ORG_ID,
        NUMBERID: t.reserveForm.SCHEDULEID,
        ...
        ZZJB: t.reserveForm.ZJBZ,
        TIMERANGE: t.reserveForm.TIMERANGE,
        YYSD: t.reserveForm.STARTTIME.substring(0,5) + '~' + t.reserveForm.ENDTIME.substring(0,5),
        STARTTIME: t.reserveForm.STARTTIME.substring(0,5),
        ENDTIME: t.reserveForm.ENDTIME.substring(0,5),
        REGFEE: t.reserveForm.REGFEE,
        TREATFEE: t.reserveForm.TREATFEE,
        ISTIMEREG: t.reserveForm.ISTIMEREG,
        PERIODID: t.reserveForm.PERIODID,
        DZJKM:t.reserveForm.DZJKM||'',  
        HOSPITALCODE:t.reserveForm.HOSPITALCODE||'',  
      } 
      var _url='' 
      if(params.ORG_ID=='hzdy001'){
        _url='dyrmyy'
      }else if(params.ORG_ID.indexOf('hzzx')!='-1'){
        _url='hzzxyy'
      }else if(params.ORG_ID=='hzfybj001'){
        _url='hzfybj'
      }else{
        _url='hzzyy'
      }
      console.log(params)
      console.log(_url)
      window.location.href = t.wx_static_api +'huizhoupay/jsapi_gh_bmfw_HuiZhou_'+_url+'.php?' + querystring.stringify(params)
      // window.location.href = wx_static_api +'huizhoupay/jsapi_gh_bmfw_HuiZhou_Common.php?' + querystring.stringify(params) 
      console.log(t.wx_static_api +'huizhoupay/jsapi_gh_bmfw_HuiZhou_'+_url+'.php?' + querystring.stringify(params))
    }
  },
  mounted() {}
}
